  ...
  try {
    ...
    if (!teamhomeid || !teamawayid || !match_date || !stadium) {
      return res.status(400).json({
        message: "teamhomeid, teamawayid, match_date, and stadium are required",
      });
    }
    if (teamhomeid === teamawayid) {
      return res
        .status(400)
        .json({ message: "A team cannot play against itself" });
    }
    ...
    if (!homeTeam || !awayTeam) {
      return res
        .status(400)
        .json({ message: "One or both teams do not exist" });
    }
...
const updateMatch = async (req, res) => {
  try {
    ...
    if (teamawayid && teamawayid !== match.teamawayid) {
      ...
      if (!away)
        return res.status(400).json({ message: "Away team does not exist" });
    }
